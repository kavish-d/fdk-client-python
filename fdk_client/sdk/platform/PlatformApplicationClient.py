...
class Discount:
    ...
    async def fetchDiscountJobs(self, ):
        """Fetch Discount Jobs.
        """
        payload = {}
        ...
        # Parameter validation
        schema = DiscountValidator.fetchDiscountJobs()
        schema.dump(schema.load(payload))
        

        url_with_params = await create_url_with_params(self._conf.domain, f"/service/platform/discount/v1.0/company/{self._conf.companyId}/application/{self.applicationId}/job/", """{"required":[{"name":"company_id","in":"path","description":"company_id","required":true,"schema":{"type":"integer"}},{"name":"application_id","in":"path","description":"application id","required":true,"schema":{"type":"string"}}],"optional":[],"query":[],"headers":[]}""", )
        query_string = await create_query_string()
        return await AiohttpHelper().aiohttp_request("GET", url_with_params, headers=await get_headers_with_signature(self._conf.domain, "get", await create_url_without_domain(f"/service/platform/discount/v1.0/company/{self._conf.companyId}/application/{self.applicationId}/job/", ), query_string, {"Authorization": "Bearer " + await self._conf.getAccessToken()}, "", exclude_headers=["Authorization"]), data="")
    
    async def getDiscountDetails(self, body=""):
        """Get Discounts.
        """
        payload = {}
        ...
        # Parameter validation
        schema = DiscountValidator.getDiscountDetails()
        schema.dump(schema.load(payload))
        
        # Body validation
        from .platform_models.InternalDiscountQuery import InternalDiscountQuery
        schema = InternalDiscountQuery()
        schema.dump(schema.load(body))
        

        url_with_params = await create_url_with_params(self._conf.domain, f"/service/platform/discount/v1.0/company/{self._conf.companyId}/application/{self.applicationId}/discount/", """{"required":[{"name":"company_id","in":"path","description":"company_id","required":true,"schema":{"type":"integer"}},{"name":"application_id","in":"path","description":"application id","required":true,"schema":{"type":"string"}}],"optional":[],"query":[],"headers":[]}""", )
        query_string = await create_query_string()
        return await AiohttpHelper().aiohttp_request("POST", url_with_params, headers=await get_headers_with_signature(self._conf.domain, "post", await create_url_without_domain(f"/service/platform/discount/v1.0/company/{self._conf.companyId}/application/{self.applicationId}/discount/", ), query_string, {"Authorization": "Bearer " + await self._conf.getAccessToken()}, body, exclude_headers=["Authorization"]), data=body)
