...
```python
try:
    result = await client.application("<APPLICATION_ID>").discount.fetchDiscountJobs()
    # use result
except Exception as e:
    print(e)
```
...
```python
try:
    result = await client.application("<APPLICATION_ID>").discount.getDiscountDetails(body=body)
    # use result
except Exception as e:
    print(e)
```
