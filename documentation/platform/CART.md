...
```python
try:
    result = await client.application("<APPLICATION_ID>").cart.getPromotions(pageNo=pageNo, pageSize=pageSize, q=q, isActive=isActive, promoGroup=promoGroup, promotionType=promotionType)
    # use result
except Exception as e:
    print(e)
```
